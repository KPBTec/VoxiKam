...
export default function BillingRecalcPage() {
  const API = '/admin/billing-recalc'
  ...
  function body() {
    ...
    // date_to es exclusivo en el backend — el último día del mes se cubre con +1 día.
  ...
  async function pollJob(jobId: string, onDone: (result: any) => void, onError: (msg: string) => void) {
    ...
    for (;;) {
      ...
      try {
        ...
        // mandó — el job puede tardar un instante en escribir su primer
        // archivo de estado. Un 404 aislado no significa que no exista,
        // solo que todavía no escribió; recién si persiste ~15s seguidos
        // (10 intentos) lo tratamos como error real. Encontrado en
        // producción real: el primer poll a los 1.5s ganaba la carrera
        // contra el propio job y mostraba "no encontrado" con el job ya
        // corriendo (y terminando bien) del otro lado.
        notFoundStreak++
  ...
  async function runApply() {
    ...
    if (!confirm(`Esto va a modificar ${preview.affected_cdrs} CDR(s) y ajustar el balance de ${preview.customers.length} cliente(s) — ¿confirmás?`)) return
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[var(--color-text-2)] mt-1">
          Recalcula el costo/venta de CDRs ya facturados contra las tarifas <strong>actuales</strong> — para cuando
          se negocia un precio nuevo con un cliente (o un carrier cambia su tarifa) y debe aplicar retroactivamente
          a llamadas ya hechas en el período. Siempre pide vista previa antes de aplicar.
        </p>
      </div>
      ...
      <div className={`${card} p-5 space-y-4`}>
        <div className="flex items-start gap-4 flex-wrap">
          ...
          <div className="flex flex-col gap-1 min-w-56">
            <span className={lbl}>{scope === 'customer' ? 'Cliente' : 'Carrier'}</span>
      ...
      {applied && (
        <div className="bg-success/10 border border-success/30 rounded-xl p-5 flex items-start gap-3">
          ...
          <div className="text-sm text-[var(--color-text)]">
            <p className="font-medium">Recálculo aplicado — {applied.affected_cdrs} CDR(s) en {applied.customers.length} cliente(s).</p>
