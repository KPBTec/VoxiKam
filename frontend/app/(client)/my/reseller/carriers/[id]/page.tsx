        ...
        {addMode === 'group' ? (
          <form onSubmit={addGroupRate} className="flex gap-3 flex-wrap items-end">
            <div className="w-52">
              ...
              <select required value={grpForm.group_name} onChange={e => setGrpForm(f => ({...f, group_name: e.target.value}))} className={input}>
            ...
            <div>
              ...
              <input required type="number" step="0.0001" min="0" placeholder="0.0000" value={grpForm.buy_rate}
          ...
          <form onSubmit={addRate} className="flex gap-3 flex-wrap items-end">
            <div className="flex-1 min-w-48">
              ...
              <select required value={rateForm.prefix_id} onChange={e => setRateForm(f => ({...f, prefix_id: e.target.value}))} className={input}>
            ...
            <div>
              <label className={lbl}>Costo/min</label>
              <input required type="number" step="0.0001" min="0" placeholder="0.0000" value={rateForm.buy_rate}
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {rates.slice().sort((a, b) => (a.group_name || '').localeCompare(b.group_name || '') || a.prefix.localeCompare(b.prefix)).map(r => (
                <tr key={r.id} className="hover:bg-white/2">
                  ...
                  <td className="px-5 py-2.5 text-right font-mono text-red-400">S/ {(+r.buy_rate).toFixed(4)}</td>
